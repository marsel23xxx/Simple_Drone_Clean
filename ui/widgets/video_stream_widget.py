...
#             rtsp_url = "rtmp://127.0.0.1:1935/live/cam1"
#         if not cam1_url:
...
# ui/widgets/video_stream_widget.py - RTMP THREADING FIX
import os
import sys
...
class RTMPCameraThread(QThread):
    """
    ✅ SOLUSI: Threading seperti kode teman untuk menghindari delay RTMP
    - grab() murah, flush queue
    - retrieve() decode sekali
    - Hanya simpan frame terbaru
    """
    ...
    def __init__(self, rtmp_url, camera_id, ai_worker=None):
        super().__init__()
        self.rtmp_url = rtmp_url
        self.camera_id = camera_id
        self.ai_worker = ai_worker
        
        # Threading state (seperti kode teman)
        self.latest = {"frame": None, "ok": False}
        self.lock = threading.Lock()
        self.running = False
        self.cap = None
        self.failed = False
        
        # Set RTMP buffer options (dari kode teman)
        os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "rtmp_transport;tcp,fflags;nobuffer,probesize;32,analyzeduration;0"
    ...
    def run(self):
        """Main thread dengan teknik grab/retrieve seperti kode teman"""
        print(f"🚀 Starting RTMP camera {self.camera_id}: {self.rtmp_url}")
        ...
        # Open RTMP stream dengan FFMPEG
        try:
            self.cap = cv.VideoCapture(self.rtmp_url, cv.CAP_FFMPEG)
            
            if not self.cap.isOpened():
                raise RuntimeError("Failed to open RTMP stream")
            
            # Set buffer size minimal (seperti kode teman)
            self.cap.set(cv.CAP_PROP_BUFFERSIZE, 1)
            
            print(f"✅ Camera {self.camera_id} connected via FFMPEG")
            self.connection_status_changed.emit("Connected")
            ...
            self.failed = True
            self.camera_failed.emit(f"Camera {self.camera_id}: {str(e)}")
            return
        ...
        # ✅ MAIN LOOP - SEPERTI KODE TEMAN
        while self.running:
            try:
                # grab() murah, flush queue (KUNCI UTAMA!)
                if not self.cap.grab():
                    consecutive_failures += 1
                    if consecutive_failures >= max_failures:
                        print(f"❌ Camera {self.camera_id} - too many grab failures")
                        self.failed = True
                        self.camera_failed.emit(f"Camera {self.camera_id}: Grab failures")
                        break
                    self.msleep(10)
                    continue
                ...
                if not ok or frame is None:
                    consecutive_failures += 1
                    if consecutive_failures >= max_failures:
                        print(f"❌ Camera {self.camera_id} - too many retrieve failures")
                        self.failed = True
                        self.camera_failed.emit(f"Camera {self.camera_id}: Retrieve failures")
                        break
                    self.msleep(10)
                    continue
                ...
                with self.lock:
                    self.latest["frame"] = frame.copy()
                    self.latest["ok"] = True
                
                # Emit frame
                self.frame_ready.emit(frame)
                
                # Small sleep setiap 100 frame
                if frame_count % 100 == 0:
                    self.msleep(1)
                
            except Exception as e:
                print(f"❌ Camera {self.camera_id} loop error: {e}")
                consecutive_failures += 1
                if consecutive_failures >= max_failures:
                    self.failed = True
                    self.camera_failed.emit(f"Camera {self.camera_id}: {str(e)}")
    ...
    def get_latest_frame(self):
        """Get latest frame (thread-safe) - seperti kode teman"""
        with self.lock:
            if self.latest["ok"]:
...
class RTSPStreamWidget(VideoStreamWidget):
    """✅ RTMP Stream Widget dengan teknik threading dari kode teman"""
    ...
    def start_single_camera(self, cam_id, url, width_scale=0.5, height_scale=0.5, enable_ai=True):
        """Start single RTMP camera dengan threading fix"""
        try:
            print(f"\n📹 Starting {cam_id} with RTMP threading...")
            ...
            # ✅ Create RTMP thread (seperti kode teman)
            camera = RTMPCameraThread(url, camera_id=cam_id, ai_worker=ai_worker)
            ...
            print(f"   ✅ {cam_id} RTMP thread started")
            return True
    ...
    def start_stream(self, rtsp_url=None, cam1_url=None, cam2_url=None, 
                     width_scale=0.5, height_scale=0.5, enable_ai=False):
        """Start RTMP stream dengan threading fix"""
        if not rtsp_url:
            rtsp_url = "rtmp://127.0.0.1:1935/live/neo"
        
        print(f"\n{'='*60}")
        print(f"🎬 STARTING RTMP STREAM WITH THREADING FIX")
        print(f"{'='*60}")
        print(f"URL: {rtsp_url}")
        print(f"AI: {'ENABLED' if enable_ai else 'DISABLED'}")
        print(f"{'='*60}\n")
        
        # Start camera 0 only
        success = self.start_single_camera("cam0", rtsp_url, width_scale, height_scale, enable_ai)
        
        if success and "cam0" in self.cameras:
            self.cameras["cam0"].frame_ready.connect(self.on_cam0_frame)
        ...
        if "cam0" in self.cameras:
            ...
            print(f"CAM0 - {status}")
            print(f"  Thread: {is_running} | Failed: {failed}")
        else:
            print(f"CAM0 - ❌ NOT STARTED")
